...
const { status, data } = await apiInstance.getProjectCluster(
    xUserID,
    id
);
```
...
const { status, data } = await apiInstance.getProjectEnvironments(
    xUserID,
    id
);
```
...
const { status, data } = await apiInstance.toggleProjectPreviewEnvironmentEnabled(
    xUserID,
    id
);
```
