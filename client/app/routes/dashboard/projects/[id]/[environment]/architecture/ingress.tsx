...
export default function Ingress() {
  ...
  const { fields: hostFields, append: appendHost } = useFieldArray({
    control,
    name: "hosts",
  });
...
function HostEditor({ control, register, hostIndex }: HostEditorProps) {
  const {
    fields: pathFields,
    append: appendPath,
    remove: removePath,
          ...
          <div key={path.id} className="relative flex flex-col gap-1">
            {pathIndex > 0 && (
              <button
                ...
                className="border-mauve-6 text-mauve-7 absolute top-4 -left-8 z-10 flex h-4 w-4 cursor-pointer items-center justify-center rounded border-[1.5px] bg-white"
              >
            ...
            <div className="border-mauve-6 relative ml-1 flex flex-col gap-1 border-l-2 pl-6">
              ...
              <div className="relative min-w-48">
                <select
                  ...
                  {...register(`hosts.${hostIndex}.paths.${pathIndex}.service`)}
                >
                  <option value="" disabled>
                    Service*
                  </option>
