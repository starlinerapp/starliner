...
func (dr *DeploymentRepository) UpdateDatabaseDeploymentCredentials(
	...
	return dr.queries.UpdateDatabaseDeploymentCredentials(ctx, sqlc.UpdateDatabaseDeploymentCredentialsParams{
		Database:     mapper.ToNullStringFromPtr(&dbName),
		Username:     mapper.ToNullStringFromPtr(&username),
		Password:     mapper.ToNullStringFromPtr(&password),
		DeploymentID: deploymentId,
	})
}
...
func (dr *DeploymentRepository) GetDeploymentWithNamespace(ctx context.Context, deploymentId int64) (*entity.Deployment, error) {
	...
	return &entity.Deployment{
		...
		EnvironmentId: mapper.ToPtrFromNullInt64(deployment.EnvironmentID),
	}, nil
}
...
func (dr *DeploymentRepository) GetDeploymentCluster(ctx context.Context, deploymentId int64) (*entity.Cluster, error) {
	...
	return &entity.Cluster{
		...
		IPv4Address:    mapper.ToPtrFromNullString(res.Ipv4Address),
		PublicKey:      mapper.ToPtrFromNullString(res.PublicKey),
		PrivateKey:     mapper.ToPtrFromNullString(res.PrivateKey),
		ProvisioningId: mapper.ToPtrFromNullString(res.ProvisioningID),
		Kubeconfig:     mapper.ToPtrFromNullString(res.Kubeconfig),
