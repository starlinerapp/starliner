...
func NewServer(
	...
	clusterRoutes := engine.Group("/clusters")
	{
		...
		clusterRoutes.POST("/:id/retry", clusterHandler.RetryCreateCluster)
	...
	githubRoutes := engine.Group("/github")
	{
		...
		githubRoutes.GET("/repositories/:organizationId/:owner/:repository/branches", githubHandler.GetRepositoryBranches)
