...
INSERT INTO projects (name, team_id, cluster_id)
    VALUES ($1, $2, $3)
RETURNING
    *;
...
WHERE
    ...
    AND EXISTS (
        ...
        WHERE
            d.environment_id = e.id
            AND g.url = $1);

-- name: GetProjectPreviewEnvironmentEnabled :one
SELECT
    p.preview_environments_enabled
FROM
    projects p
    INNER JOIN teams t ON t.id = p.team_id
    INNER JOIN team_members tm ON tm.team_id = t.id
...
UPDATE
    projects p
SET
    preview_environments_enabled = NOT p.preview_environments_enabled
FROM
    teams t
    INNER JOIN team_members tm ON tm.team_id = t.id
WHERE
