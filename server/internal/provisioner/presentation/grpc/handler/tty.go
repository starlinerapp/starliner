...
type TtyHandler struct {
	v1.UnimplementedClusterTTYServiceServer
	ttyApplication *application.TTYApplication
}

func NewTtyHandler(ttyApplication *application.TTYApplication) *TtyHandler {
	return &TtyHandler{ttyApplication: ttyApplication}
}

func (th *TtyHandler) OpenTTY(stream v1.ClusterTTYService_OpenTTYServer) error {
	ctx := stream.Context()
	...
	session, ok := firstReq.Payload.(*v1.OpenClusterTTYRequest_Session)
	if !ok {
	...
	go func() {
		...
		for {
			...
			if n > 0 {
				_ = stream.Send(&v1.OpenClusterTTYResponse{Stdout: buf[:n]})
			}
			if err != nil {
				if err != io.EOF {
					_ = stream.Send(&v1.OpenClusterTTYResponse{Stdout: []byte(fmt.Sprintf("error: %v", err))})
				}
				return
			}
		}
